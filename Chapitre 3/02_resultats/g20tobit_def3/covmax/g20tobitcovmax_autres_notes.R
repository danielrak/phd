# g20tobitcovmax (def3). autres notes. 
  # pertinent pour economie uniquement (voir distribution de la note de gestion et economie)
  # mais on fait quand même les deux. 
  # censuré à gauche uniquement.
...
#     for (k in g20depvars) {
#       for (l in g20dats) {
#         assign(paste("g20tobit_", i, "_", j %>% str_remove("g20"), 
#                      "_", k, "_tous_", l, sep = ""),
#                tobit1(as.formula(
#                  paste(
#                    k, " ~ ", 
...
for (i in g20treats) {
  for (j in g20specs) {
    for (k in g20depvars) {
      for (l in g20dats) {
        assign(paste("g20tobitl_", i, "_", j %>% str_remove("g20"), 
                     "_", k, "_tous_", l, sep = ""),
               tobit1(as.formula(
                 paste(
                   k, " ~ ", 
...
for (i in ls()[str_detect(ls(), "^g20tobit")]) {
  
  assign(paste("ct.", i, sep = ""),
         coef(summary(get(i)))) # à transformer en écart-types robustes. 
...
save(list = ls()[str_detect(ls(), "g20tobit") & 
                   ! str_detect(ls(), "^g20tobit")],
     file = here("g20tobit_def3", "covmax",
                 "g20tobitcovmax_autres_notes.rda"),
     version = 2)
