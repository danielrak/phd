# agecfh_pcs_g2_reseau_bin_sep_c101112 (def2). 
# fee : effets école.
...
load(here("agepe_def2", "agepe_reseau_bin_sep_c101112.rda"))
  ...
  assign(paste("agecfh_pcs_g2_", 
         ...
         c %>% filter(cohorte != "2009") %>% 
           ...
           mutate(resid = eval(parse(text = paste(
                    "resid.agepe_z_reseau_bin_", j, "_c101112", 
                    sep = ""
                  ))))))
}
}
...
save(list = ls()[str_detect(ls(), "agecfh") & 
     ...
     file = here("agecfh_def2",
                 "agecfh_pcs_g2_reseau_bin_sep_c101112.rda"),
     version = 2)