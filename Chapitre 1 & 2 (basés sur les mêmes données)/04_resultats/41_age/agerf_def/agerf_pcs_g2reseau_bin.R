...
for(j in agepcs_g2reseau_bin) {
  for (i in agedepvars_c) {
    assign(paste("agerf_",
           ...
           c %>% filter(pcs_g2 == j %>% str_extract(".*_AND_") %>% 
                          str_remove("_AND_") & 
                          reseau_bin == j %>% 
                          str_extract("_AND_.*") %>% 
                          str_remove("_AND_")),
...
for(j in agepcs_g2reseau_bin) {
  for (i in agedepvars_cdi) {
    assign(paste("agerf_",
           ...
           cdi %>% filter(pcs_g2 == j %>% str_extract(".*_AND_") %>% 
                            str_remove("_AND_") & 
                            reseau_bin == j %>% 
                            str_extract("_AND_.*") %>% 
                            str_remove("_AND_")),
...
for (j in agepcs_g2reseau_bin) {
  for (i in agedepvars_cdi_w_scores_cm2) {
    ...
    cdi %>% 
      filter(pcs_g2 == j %>% str_extract(".*_AND_") %>% 
               str_remove("_AND_") & 
               reseau_bin == 
               j %>% str_extract("_AND_.*") %>% 
               str_remove("_AND_")),
...
dir.create(here("agerf_def", "agerf_pcs_g2reseau_bin"))
