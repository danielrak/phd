...
z_pcs <- paste("z", c("pcs_g2_Defav", "pcs_g2_Moy",
                      "pcs_g2_Fav", "pcs_g2_Tresfav",
                      "pcs_g2_Autres", "pcs_g2_NA",
                      
                      # full interaction nécessaire. 
                      "sexe_F", "sexe_M", 
                      "cohorte_2009", "cohorte_2010", "cohorte_2011",
                      "cohorte_2012"
                      ), sep = " : ")

z_pcs_cdi <- paste("z", c("pcs_g2_Defav", "pcs_g2_Moy",
                      "pcs_g2_Fav", "pcs_g2_Tresfav",
                      "pcs_g2_Autres", "pcs_g2_NA",
...
agerf_rp_pcs <- plm(as.formula(paste(depvar, " ~ ", 
                                  z_pcs %>% paste(collapse = " + "), " + ",
                                  cont %>% paste(collapse = " + "), " + ",
                                  coh %>% paste(collapse = " + "))),
...
for (i in c("score_f_rp", "ecrire_rp", "grammaire_rp", "lire_rp", 
            ...
            "org_donnee_rp")) assign(
              paste("agerf_rp_pcs_", 
                    ...
                    sep = ""),
              update(agerf_rp_pcs, as.formula(paste(i, " ~ .", sep = "")))
            )

# Sur cdi -----------------------------------------------------------------

agerf_rp_pcs_cdi <- plm(as.formula(paste(depvar_cdi, " ~ ", 
                                  z_pcs_cdi %>% paste(collapse = " + "), " + ",
                                  cont %>% paste(collapse = " + "), " + ",
...
for (i in c("moy_fran_ec_rp", "moy_maths_ec_rp")) assign(
              paste("agerf_rp_pcs_", 
                    i, "_cdi",  
                    sep = ""),
              update(agerf_rp_pcs_cdi, as.formula(paste(i, " ~ .", sep = "")))
            )
...
save(list = ls()[str_detect(ls(), "^ct\\.|^n\\.|^arsq\\.|^mvc\\.")], 
     file = here("agerf_rp", "agerf_rp_pcs.rda"), version = 2)