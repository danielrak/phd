...
for (i in agedepvars_c %>% 
  ...
  assign(paste("ageiv_excl_sexe_", 
               i, 
               "_tous_c",
               sep = ""),
         ivreg(as.formula(paste(
           i, " ~ ", 
           c("age_abs", "sexe", "age_abs : sexe"
             , "pcsR", "cohorte",
             "mois_2", "mois_3", "mois_4", "mois_5",
             "mois_6", "mois_7", "mois_8", "mois_9", 
             "mois_10", "mois_11") %>% 
             paste(collapse = " + "),
           "|. - age_abs + mois_1 - age_abs : sexe + mois_1 : sexe", 
           sep = ""
         )),
         data = c))
}
...
save(list = ls()[str_detect(ls(), "ageiv") & 
                   ! str_detect(ls(), "^ageiv")],
     file = here("ageiv_def2", "ageiv_excl_sexe.rda"),
     version = 2)
