# agesrd_sexepcs_g2reseau_bin (def).
...
for (j in agesexepcs_g2reseau_bin[
  ! str_detect(agesexepcs_g2reseau_bin, "Autres")
  # pbm nobs trop faible. 
]) {
  for (i in agedepvars_c) {
    
    assign(paste("agesrd_p1_", i, 
                 "_sexe_", j %>% str_extract(".*_AND_") %>% 
                   str_remove("_AND_"),
                 "_pcs_g2_", j %>% 
                   str_extract("_AND_.*_AND2_") %>% 
                   str_remove("_AND_") %>%
                   str_remove("_AND2_"),
                 "_reseau_bin", j %>% 
                   str_extract("_AND2_.*") %>% 
...
dir.create(here("agesrd_def", "agesrd_sexepcs_g2reseau_bin"))
