# pes13hnolinq5corrnppsfe_pcs_reg_mod. (def). 
...
for (i in pedepvars %>% 
  ...
  assign(paste("pes13hnolinq5corrnppsfe_pcs_reg_mod_", j, "_",
               i,
               "_tous_", k,
               sep = ""),
         feols(as.formula(paste(
           i, " ~ ",
           c(peersindivsnp_q5_pcs_reg_mod, "score_norm",
             get(j)) %>% 
             paste(collapse = " + "),
...
save(list = ls()[str_detect(ls(), "pes13hnolinq5corr") & 
     ...
     file = here("pes13hnolinq5corrnppsfe_def", 
                 "pes13hnolinq5corrnppsfe_pcs_reg_mod.rda"),
     version = 2)
