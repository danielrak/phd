# pes13h5corr_pcs_reg_mod. (def).
  # corr : score en var expl. MAJ2022_02_09.
...
for (i in pedepvars %>% 
  ...
  assign(paste("pes13h5corr_pcs_reg_mod_", j, "_",
               i,
               "_tous_", k,
               sep = ""),
         plm(as.formula(paste(
           i, " ~ ",
           c("(p_score_norm : p : q5score) * pcs_reg_mod", "score_norm",
             get(j)) %>% 
             paste(collapse = " + ")
         )),
         data = get(k), 
...
save(list = ls()[str_detect(ls(), "pes13h5corr") & 
     ...
     file = here("pes13h5corr_def", "pes13h5corr_pcs_reg_mod.rda"),
     version = 2)
