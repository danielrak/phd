...
for (i in pedepvars %>% 
  ...
  assign(paste("pes13h5nofe_pcs_reg_mod_", j, "_",
               i,
               "_tous_", k,
               sep = ""),
         lm(as.formula(paste(
           i, " ~ ",
           c("(p_score_norm : p : q5score) * pcs_reg_mod", "q5score",
             get(j)) %>% 
             paste(collapse = " + ")
         )),
         data = get(k)))
...
for (i in ls()[str_detect(ls(), "^pes13h5nofe")]) {
...
save(list = ls()[str_detect(ls(), "pes13h5nofe") & 
                  ! str_detect(ls(), "^pes13h5nofe")],
     file = here("pes13h5nofe_def", "pes13h5nofe_pcs_reg_mod.rda"),
     version = 2)
