...
for (i in pedepvars %>% 
  ...
  assign(paste("pes13h5nofe_sexe_mod_", j, "_",
               i,
               "_tous_", k,
               sep = ""),
         lm(as.formula(paste(
           i, " ~ ",
           c("(p_score_norm : p : q5score) * sexe_mod", "q5score",
             get(j)) %>% 
             paste(collapse = " + ")
         )),
         data = get(k)))
...
save(list = ls()[str_detect(ls(), "pes13h5nofe") & 
     ...
     file = here("pes13h5nofe_def", "pes13h5nofe_sexe_mod.rda"),
     version = 2)
