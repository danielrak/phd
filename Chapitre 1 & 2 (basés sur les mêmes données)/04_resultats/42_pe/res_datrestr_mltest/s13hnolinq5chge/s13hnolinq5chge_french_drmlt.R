...
s13hnolinq5chge_sA_french_drmlt <- 
...
s13hnolinq5chge_sB_french_drmlt <- 
  plm(as.formula(paste(depvar, " ~ ", 
                       indivs %>% paste(collapse = " + "), " + ", 
                       peersindivs %>% paste(collapse = " + "), " + ",
                       sB %>% paste(collapse = " + "),
...
s13hnolinq5chge_sC_french_drmlt <- 
  plm(as.formula(paste(depvar, " ~ ", 
                       indivs %>% paste(collapse = " + "), " + ", 
                       peersindivs %>% paste(collapse = " + "), " + ",
                       sC %>% paste(collapse = " + "),
...
s13hnolinq5chge_sD_french_drmlt <- 
  plm(as.formula(paste(depvar, " ~ ", 
                       indivs %>% paste(collapse = " + "), " + ", 
                       peersindivs %>% paste(collapse = " + "), " + ",
                       sD %>% paste(collapse = " + "),
...
save(list = ls()[str_detect(ls(), "^ct\\.|^n\\.|^arsq\\.|^mvc\\.")], 
     file = here("res_datrestr_mltest", "s13hnolinq5chge", 
                 "s13hnolinq5chge_french_drmlt.rda"), version = 2)