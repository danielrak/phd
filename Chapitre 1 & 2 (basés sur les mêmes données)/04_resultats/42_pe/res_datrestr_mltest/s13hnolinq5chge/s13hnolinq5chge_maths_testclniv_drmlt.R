# s13hnolinq5chge, maths, testclniv
...
s13hnolinq5chge_sD_maths_wclnivsup_drmlt <- 
...
s13hnolinq5chge_sD_maths_wclnivinf_drmlt <- 
  plm(as.formula(paste(depvar, " ~ ", 
                       indivs %>% paste(collapse = " + "), " + ", 
                       peersindivs %>% paste(collapse = " + "), " + ",
                       sD %>% paste(collapse = " + "),
...
save(list = ls()[str_detect(ls(), "^ct\\.|^n\\.|^arsq\\.")], 
     file = here("res_datrestr_mltest", "s13hnolinq5chge",
                 "s13hnolinq5chge_maths_testclniv_drmlt.rda"), version = 2)