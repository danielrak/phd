...
peersindivs <- paste("p_q5score_tot_q", (1:5)[- 3], 
                     sep = "") %>% 
...
s13olshnolinq5_sA_french_drmltest <- 
...
s13olshnolinq5_sB_french_drmltest <- 
  plm(as.formula(paste(depvar, " ~ ", 
                       indivs %>% paste(collapse = " + "), " + ", 
                       peersindivs %>% paste(collapse = " + "), " + ",
                       sB %>% paste(collapse = " + "),
...
s13olshnolinq5_sC_french_drmltest <- 
  plm(as.formula(paste(depvar, " ~ ", 
                       indivs %>% paste(collapse = " + "), " + ", 
                       peersindivs %>% paste(collapse = " + "), " + ",
                       sC %>% paste(collapse = " + "),
...
s13olshnolinq5_sD_french_drmltest <- 
  plm(as.formula(paste(depvar, " ~ ", 
                       indivs %>% paste(collapse = " + "), " + ", 
                       peersindivs %>% paste(collapse = " + "), " + ",
                       sD %>% paste(collapse = " + "),
...
save(list = ls()[str_detect(ls(), "^ct\\.|^n\\.|^arsq\\.|^mvc\\.")], 
     file = here("res_datrestr_mltest", "s13olshnolinq5", 
                 "s13olshnolinq5_french_drmltest.rda"), version = 2)