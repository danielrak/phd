...
s13npchge_sA_hist_o1_drmlt <- plm(moy_hist_ec ~ 
                                pnp_score : p + pop_score : p + v_ob_tot + p, 
                              datrestr_mltest, subset = obs == 1, index = "rneconstatses")

# sB ----------------------------------------------------------------------

s13npchge_sB_hist_o1_drmlt <- update(s13npchge_sA_hist_o1_drmlt, . ~ . 
...
s13npchge_sC_hist_o1_drmlt <- update(s13npchge_sB_hist_o1_drmlt, . ~ . + tdivconstatrneses)

# sD ----------------------------------------------------------------------

s13npchge_sD_hist_o1_drmlt <- update(s13npchge_sC_hist_o1_drmlt, . ~ . 
...
s13npchge_sE_hist_o1_drmlt <- update(s13npchge_sD_hist_o1_drmlt, . ~ . 
...
s13npchge_sF_hist_o1_drmlt <- update(s13npchge_sC_hist_o1_drmlt, . ~ . 
...
save(list = ls()[str_detect(ls(), "^ct\\.|^n\\.|^arsq\\.|^mvc\\.")], 
     file = here("res_datrestr_mltest", "s13npchge", 
                 "s13_hist_o1_drmlt.rda"), version = 2)