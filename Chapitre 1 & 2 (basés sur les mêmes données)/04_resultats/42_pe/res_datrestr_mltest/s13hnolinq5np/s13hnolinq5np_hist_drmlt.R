...
s13hnolinq5np_sA_hist_drmlt <- plm(moy_hist_ec ~ 
...
s13hnolinq5np_sB_hist_drmlt <- update(s13hnolinq5np_sA_hist_drmlt, . ~ . 
...
s13hnolinq5np_sC_hist_drmlt <- update(s13hnolinq5np_sB_hist_drmlt, . ~ . + tdivconstatrneses)

# sD ----------------------------------------------------------------------

s13hnolinq5np_sD_hist_drmlt <- update(s13hnolinq5np_sC_hist_drmlt, . ~ . 
...
s13hnolinq5np_sE_hist_drmlt <- update(s13hnolinq5np_sD_hist_drmlt, . ~ . 
...
s13hnolinq5np_sF_hist_drmlt <- update(s13hnolinq5np_sC_hist_drmlt, . ~ . 
...
save(list = ls()[str_detect(ls(), "^ct\\.|^n\\.|^arsq\\.")], 
     file = here("res_datrestr_mltest", "s13hnolinq5np",
                 "s13hnolinq5np_hist_drmlt.rda"), version = 2)