...
s13h5chge_sD_french_wclnivsup_drmlt <- 
...
s13h5chge_sD_french_wclnivinf_drmlt <- 
  plm(as.formula(paste(depvar, " ~ ", 
                       indivs %>% paste(collapse = " + "), " + ", 
                       peersindivs %>% paste(collapse = " + "), " + ",
                       sD %>% paste(collapse = " + "),
...
save(list = ls()[str_detect(ls(), "^ct\\.|^n\\.|^arsq\\.|^mvc\\.")], 
     file = here("res_datrestr_mltest", "s13h5chge", 
                 "s13h5chge_french_testclniv_drmlt.rda"), version = 2)