...
for (i in pedepvars %>% 
  ...
  assign(paste("pes13hbcorr_pcs_reg_mod_", j, "_",
               i,
               "_tous_", k,
               sep = ""),
         plm(as.formula(paste(
           i, " ~ ",
           c("(p_score_norm : p : qbscore) * pcs_reg_mod", 
             "score_norm",
             get(j)) %>% 
             paste(collapse = " + ")
         )),
...
#          plm(as.formula(paste(
#            i, " ~ ",
#            c("(p_score_norm : p : qbscore) * 
#              pcs_reg_mod", "score_norm",
#              get(j)) %>% 
#              paste(collapse = " + ")
#          )),
#          data = get(k) %>% 
...
for (i in ls()[str_detect(ls(), "^pes13hbcorr")]) {
...
save(list = ls()[str_detect(ls(), "pes13hbcorr") & 
                  ! str_detect(ls(), "^pes13hbcorr")],
     file = here("pes13hbcorr_def", 
                 "pes13hbcorr_pcs_reg_mod.rda"),
     version = 2)
