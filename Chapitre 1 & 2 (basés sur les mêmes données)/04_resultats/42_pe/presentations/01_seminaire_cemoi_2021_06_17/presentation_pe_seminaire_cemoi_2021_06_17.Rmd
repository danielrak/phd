...
subtitle : "Présentation au séminaire du CEMOI"
author : "Daniel Rakotomalala"
date : "17 Juin 2021"
institute : "Centre d'Économie et de Management de l'Océan Indien"
safe-columns : true
...
alabel <- as_labeller(c("q1" = "Q1", "q2" = "Q2",
                        "q3" = "Q3", "q4" = "Q4",
                        "q5" = "Q5",
                        "non" = "Nouveaux pairs : non", "oui" = "Nouveaux pairs : oui"))
...
-   Un élève de 3^ème^ du système français passe 26 heures par semaines avec ses camarades de classe (ses \textcolor{red}{pairs}) [^1]. \vfill
...
-   Intérêts :
    ...
    -   Complète la connaissance relativement limitée sur cette question, pour le cas de la France [@MON:eal:19]. \vfill
    ...
    -   Contribue au débat sur les politiques de \textcolor{red}{mixité scolaire} : faut-il former des classes de niveau hétérogènes ou homogènes ? 
...
\begin{tikzpicture}[->, >= stealth']
  ...
  \node (7) at (- 2, 2) [align = center] {Observables \\ de l'indiv\\ \scriptsize (Sexe, CSP)} ; 
...
- $\Rightarrow$ Si on observe une corrélation entre la note des pairs et la note de l'élève, il est impossible de connaître qui a influencé qui [@ANG:14].
...
- Exemple : l'effet de la qualité de l'enseignant (effet corrélé de classe) peut se confondre avec l'effet exclusif des pairs. 
- Exemple : l'effet des moyens matériels de l'établissement (effet corrélé d'établissement) peut se confondre avec l'effet exclusif des pairs. 
...
- $\Rightarrow$ Difficile de séparer l'effet de la note des pairs de l'effet de la catégorie sociale. 
...
- Effets du niveau d'entrée [@LAV:eal:12 ; @BOU:MAI:18 ; @SOJ:13, par exemple.] : intérêt = politiques de mixité scolaire.
...
kable2(lit %>% (function (d) {
  d[, 3] <- paste("\\textbf{\\textcolor{red}{", d[, 3], "}}", sep = "")
  d[, 5] <- paste("\\textbf{\\textcolor{red}{", d[, 5], "}}", sep = "")
  d
...
kable2(lit %>% (function (d) {
  d[c(1, 11), c(1, 2, 3, 6)] <- apply(d[c(1, 11), c(1, 2, 3, 6)], 2, function (x) paste("\\textbf{\\textcolor{red}{", x, "}}", sep = ""))
  d
})) %>% kable_styling(font_size = 4.5) %>% 
  footnote2(general = c("\\\\textit{Sources :} Papiers cités, auteur.",
                        "\\\\textit{Notes :} LEM : linéaire en moyennes. UET : unité d'écart-type. RP : rang de percentiles."))
...
\begin{tikzpicture}[->, >= stealth']
  ...
  \draw [->] (2) to node[midway, above, align = left] {\tiny Récupération des notes au CM2 (\textcolor{red}{78\% retrouvés}). \\ \tiny Pertes à cause de la limite des données \\ \tiny + redoublement/saut de classe entre CM2 et 3ème. \\ \\ \tiny Récupération des classes au CM2. \\ \tiny (\textcolor{red}{anciens/nouveaux pairs}).} (1) ;  
  \draw[->] (3) to node[midway, below right, align = left] {\\ \tiny Récupération des classes en 3ème en début d'année. \\ \tiny (\textcolor{red}{changements d'écoles : 1.2\%}).} (1) ; 
\end{tikzpicture}
```
...
lapply(list(datcompl, 
       ...
       function (x) {
         ...
         c("nb_ecole" = length(unique(x$rneconstat)),
           ...
           "tcl" = paste(mean(x$tdivconstatrneses) %>% round(3),
                         " (", sd(x$tdivconstatrneses) %>% round(3), ")", sep = ""),
           
           "notednb" = paste(mean(x$moyG1, na.rm = TRUE) %>% round(3), 
                             " (", sd(x$moyG1, na.rm = TRUE) %>% round(3), ")", 
                             sep = ""),
           " " = "",
           ...
           "notecm2" = paste(mean(x$score, na.rm = TRUE) %>% round(3),
                             " (", sd(x$score, na.rm = TRUE) %>% round(3), ")",
                             sep = ""),
           
           " " = "",
           
           "pnp" = group_by(x, divconstatrneses, tdivconstatrneses, classecoh) %>% 
             summarise(op = n()) %>% mutate(top = op / tdivconstatrneses) %>% pull(top) %>% (function (x) 1 - mean(x)) %>% round(3),
           "notecm2np" = paste(mean(x$pnp_score, na.rm = TRUE) %>% round(3), " (", sd(x$pnp_score, na.rm = TRUE) %>% round(3), ")", sep = ""),
           "notecm2op" = paste(mean(x$pop_score, na.rm = TRUE) %>% round(3), " (", sd(x$pop_score, na.rm = TRUE) %>% round(3), ")", sep = ""),
           
           " " = "",
           
           "pchge" = mean(x$chge) %>% round(3),
           "notecm2chge" = x[x$chge == 1, "score"][[1]] %>% na.omit %>% (function (x) 
             paste(mean(x) %>% round(3), " (", sd(x) %>% round(3), ")", sep = "")),
           "notecm2nochge" = x[x$chge == 0, "score"][[1]] %>% na.omit %>% (function (x) 
             paste(mean(x) %>% round(3), " (", sd(x) %>% round(3), ")", sep = "")),
  ...
  (function (d) {
    rownames(d) <- rownames(d) %>% str_replace("nb_ecole", "Nombre d'établissements") %>% str_replace("nb_classe", "Nombre de classes par établissement") %>%
      ...
      str_replace("^notecm2$", "Note au CM2") %>% str_replace("pnp", "Proportion de nouveaux pairs") %>% str_replace("notecm2np", "Note au CM2 des nouveaux pairs") %>% 
      str_replace("notecm2op", "Note au CM2 des anciens pairs") %>% str_replace("pchge", "Proportion de changements d'établissement") %>% 
      str_replace("notecm2chge", "Note au CM2, changement d'établissement") %>% str_replace("notecm2nochge", "Note au CM2, pas de changement d'établissement") %>% 
  ...
  kable2 %>% kable_styling0 %>% footnote2(general = c(
    "\\\\tiny \\\\textit{Sources :} Fichiers DNB (2014 à 2016), fichiers CM2 (2010 à 2012), fichiers CONSTAT, calculs de l'auteur.",
    "\\\\tiny \\\\textit{Notes :} Toutes les notes sont exprimées en rang de percentiles. Les moyennes de notes au CM2 sont calculées sur les retrouvés uniquement." ,
    "\\\\tiny Colonne (3) : échantillon principal des estimations. Écart-types entre parenthèses. KS : Kolmogorov-Smirnov."))
```
...
lapply(list(datcompl, 
  ...
  set_colnames(c("Données complètes", "(1) sans classes atypiques", "(2) après tests KS") %>% col_numbers) %>% 
  (function (d) {d[, 2:3] <- d[, 2:3] %>% apply(2, function (x) paste("\\textcolor{red}{", x, "}", sep = "")) ; d}) %>% 
  kable2 %>% kable_styling0 %>% footnote2(general = c(
    "\\\\tiny \\\\textit{Sources :} Fichiers DNB (2014 à 2016), fichiers CM2 (2010 à 2012), fichiers CONSTAT, calculs de l'auteur.",
    "\\\\tiny \\\\textit{Notes :} Toutes les notes sont exprimées en rang de percentiles. Les moyennes de notes au CM2 sont calculées sur les retrouvés uniquement." ,
    "\\\\tiny Colonne (3) : échantillon principal des estimations. Écart-types entre parenthèses. KS : Kolmogorov-Smirnov."))
...
lapply(list(datcompl, 
  ...
  set_colnames(c("Données complètes", "(1) sans classes atypiques", "(2) après tests KS") %>% col_numbers) %>% 
  (function (d) {
    d[c(9, 13), ] <- d[c(9, 13), ] %>% apply(2, function (x) paste("\\textcolor{red}{", x, "}", sep = ""))
    d}) %>% 
  kable2 %>% kable_styling0 %>% footnote2(general = c(
    "\\\\tiny \\\\textit{Sources :} Fichiers DNB (2014 à 2016), fichiers CM2 (2010 à 2012), fichiers CONSTAT, calculs de l'auteur.",
    "\\\\tiny \\\\textit{Notes :} Toutes les notes sont exprimées en rang de percentiles. Les moyennes de notes au CM2 sont calculées sur les retrouvés uniquement." ,
...
- La part des retrouvés, la proportion de nouveaux pairs et la proportion d'élèves qui changent d'école ne dépendent pas du type de classes. 
...
```{r}
mimicer2(list(ct.s13chge_sD_o1_dr2, ct.s13npchge_sE_o1_dr2) %>% lapply(function (x) x[str_detect(rownames(x), "v\\_ob") | str_detect(rownames(x), "pnp_score"), ] %>% 
                                                                     (function (x) {rownames(x) <- str_replace(rownames(x), "p:pnp_score", "p_v_ob_tot:p") ; x}))) %>% 
  rbind(c("n", c(n.s13chge_sD_o1_dr2, n.s13npchge_sE_o1_dr2) %>% round(3))) %>% rbind(c("arsq", c(arsq.s13chge_sD_o1_dr2, arsq.s13npchge_sE_o1_dr2) %>% round(3))) %>% 
  (function (d) {
  ...
  (function (d) {
    rbind(d[1:6, ], 
          c("Indicateurs de quintiles individuels", "\\ding{52}", "\\ding{52}"),
          c("Effets établissement-année", "\\ding{52}", "\\ding{52}"),
          c("Contrôles individuels", "\\ding{52}", "\\ding{52}"), 
          c("Taille de classe", "\\ding{52}", "\\ding{52}"), 
          c("Contrôles chez les pairs", "\\ding{52}", "\\ding{52}"), 
          c("Changement d'école", "\\ding{52}", "\\ding{52}"),
          c("Nouveaux pairs/anciens pairs", "\\ding{56}", "\\ding{52}"),
          rep("", ncol(d)),
          d[7:nrow(d), ])
  }) %>% 
  set_colnames(c("", "(1)", "(2)")) %>% 
  kable2 %>% kable_styling2 %>% 
  add_header_above(c("", "Variable dépendante : note au DNB" = 2)) %>% 
  footnote2(general = c("\\\\fontsize{4pt}{4pt}\\\\selectfont \\\\textit{Sources :} Fichiers DNB(2014 à 2016), fichiers CM2 (2010 à 2012) et fichiers CONSTAT, calculs de l'auteur.",
                       "\\\\fontsize{4pt}{4pt}\\\\selectfont \\\\textit{Notes :} Les notes sont exprimées en rang de percentiles. La variable Note CM2 pairs n'est pas la même dans (1) et (2) mais est présentée ainsi par souci de visualisation. Dans (2), il s'agit de la note au CM2 des nouveaux pairs.",
                       "\\\\fontsize{4pt}{4pt}\\\\selectfont Significativité : 10\\\\% * 5\\\\% ** 1\\\\% ***. Écart-types robustes entre parenthèses."))
```

\scriptsize

- 1 RP note CM2 pairs $\rightarrow$ 0.2 RP ($\approx$ 1/3 coefficient de la note au CM2) note DNB. 
- L'effet passe à 0.35 RP ($\approx$ 1/2 coefficient de la note au CM2) lorsque l'on prend en compte les nouveaux/anciens pairs. 
...
- (2) : De même ordre de grandeur que les coefficients de @SOJ:13. Mais pas @BOU:MAI:18 : niveau d'éducation ? territoire ?. 

## Résultats : effets de pairs hétérogènes

```{r}
mimicer2(list(ct.s13h5chge_sD_dr2, ct.s13h5npchge_sE_dr2) %>% lapply(function (x) x[str_detect(rownames(x), "v\\_ob") |str_detect(rownames(x), "pnp_score"), ] %>% 
                                                                   (function (x) {
  ...
  rbind(c("n", c(n.s13h5chge_sD_dr2, n.s13h5npchge_sE_dr2) %>% round(3))) %>% rbind(c("arsq", c(arsq.s13h5chge_sD_dr2, arsq.s13h5npchge_sE_dr2) %>% round(3))) %>% 
  (function (d) {
  ...
  (function (d) {
        rbind(d[1:12, ], 
          ...
          rep("", ncol(d)),
          d[13:nrow(d), ])
  }) %>% set_colnames(c("", "(1)", "(2)")) %>% kable2 %>% kable_styling3 %>% 
  add_header_above(c("", "Variable dépendante : note au DNB" = 2)) %>% 
  footnote2(general = c("\\\\fontsize{4pt}{4pt}\\\\selectfont \\\\textit{Sources :} Fichiers DNB(2014 à 2016), fichiers CM2 (2010 à 2012) et fichiers CONSTAT, calculs de l'auteur.",
                       "\\\\fontsize{4pt}{4pt}\\\\selectfont \\\\textit{Notes :} Les notes sont exprimées en rang de percentiles. Les quintiles sont calculés sur les candidats au DNB, par année. La variable Note CM2 pairs n'est pas la même dans (1) et (2) mais est présentée ainsi par souci de visualisation.",
                       "\\\\fontsize{4pt}{4pt}\\\\selectfont Dans (2), il s'agit de la note au CM2 des nouveaux pairs. Significativité : 10\\\\% * 5\\\\% ** 1\\\\% ***. Écart-types robustes entre parenthèses."))
```

\scriptsize

- Effet croissant de manière monotone avec le niveau au CM2 jusqu'à Q4. 
- Effets dans (2)  > Effets dans (1). 
...
- Pas en accord avec la majorité des papiers, sauf @DUF:eal:11 (Kenya) : contexte socio-économique similaire ?. 
...
```{r}
rbind(filter(hnolintab_q5_chge_dr2, depvar == "tot") %>% mutate(npchge = "non"), 
      filter(hnolintab_q5_npchge_dr2, depvar == "tot") %>% mutate(npchge = "oui", peer = paste("p", peer, sep = ""))) %>% 
  ggplot(aes(peer, coef / 10)) + geom_point() + geom_line(aes(group = indiv)) + geom_errorbar(aes(ymin = low / 10, ymax = up / 10), width = .1) + facet_grid(npchge ~ indiv, labeller = alabel) + 
  geom_hline(aes(yintercept = 0), linetype = "dashed", color = "red") + labs %>% 
  (function (l) {
    l$caption = "Sources : Fichiers DNB (2014 à 2016), fichiers CM2 (2010 à 2012), fichiers, calculs de l'auteur. \nNotes : Les quintiles sont calculées par année (DNB) sur les données complètes. N = 26015 dans les deux lignes de panels. \nIntervalles de confiance à 95% à partir d'écart-types robustes." 
...
filter(hnolintab_q5_npchge_dr2, depvar == "tot") %>% mutate(npchge = "oui", peer = paste("p", peer, sep = "")) %>% 
  ...
  geom_hline(aes(yintercept = 0), linetype = "dashed", color = "red") + labs %>% 
  (function (l) {
    l$caption = "Sources : Fichiers DNB (2014 à 2016), fichiers CM2 (2010 à 2012), fichiers CONSTAT, calculs de l'auteur. \nNotes : Les quintiles sont calculées par année (DNB) sur les données complètes. N = 26015 dans les deux lignes de panels. \nIntervalles de confiance à 95% à partir d'écart-types robustes." 
    l}) + scale_x_discrete(labels = peerlab) + theme_bw() + mytheme
```
...
- Regrouper \textcolor{red}{Q3, Q4, Q5}S équitablement entre eux. 
...
- Limites (liste non exhaustive) : 
  ...
  -   Pas de données sur les enseignants (biais de surestimation si les meilleurs enseignants affectés aux classes de bon niveau).
