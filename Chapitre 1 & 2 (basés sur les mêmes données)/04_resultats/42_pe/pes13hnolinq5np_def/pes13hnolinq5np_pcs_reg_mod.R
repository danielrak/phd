# pes13hnolinq5np_pcs_reg_mod. (def). 
...
for (i in pedepvars %>% 
  ...
  assign(paste("pes13hnolinq5np_pcs_reg_mod_", j, "_",
               i,
               "_tous_", k,
               sep = ""),
         plm(as.formula(paste(
           i, " ~ ",
           c(peersindivsnp_q5_pcs_reg_mod, "q5score",
             get(j)) %>% 
             paste(collapse = " + ")
         )),
         data = get(k), 
...
for (i in ls()[str_detect(ls(), "^pes13hnolinq5")]) {
...
save(list = ls()[str_detect(ls(), "pes13hnolinq5") & 
     ...
     file = here("pes13hnolinq5np_def", 
                 "pes13hnolinq5np_pcs_reg_mod.rda"),
     version = 2)
