# pes13hnolinqbcorr_pcs_reg_mod. (def). 
...
for (i in pedepvars %>% 
  ...
  assign(paste("pes13hnolinqbcorr_pcs_reg_mod_", j, "_",
               i,
               "_tous_", k,
               sep = ""),
         plm(as.formula(paste(
           i, " ~ ",
           c(peersindivs_qb_pcs_reg_mod, "score_norm",
             get(j)) %>% 
             paste(collapse = " + ")
         )),
         data = get(k), 
...
save(list = ls()[str_detect(ls(), "pes13hnolinqbcorr") & 
     ...
     file = here("pes13hnolinqbcorr_def", 
                 "pes13hnolinqbcorr_pcs_reg_mod.rda"),
     version = 2)
