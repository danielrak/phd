...
for (i in pedepvars %>% 
  ...
  assign(paste("pes13hnolinq5_sexe_mod_", j, "_",
               i,
               "_tous_", k,
               sep = ""),
         plm(as.formula(paste(
           i, " ~ ",
           c(peersindivs_q5_sexe_mod, "q5score",
             get(j)) %>% 
             paste(collapse = " + ")
         )),
         data = get(k), 
...
save(list = ls()[str_detect(ls(), "pes13hnolinq5") & 
     ...
     file = here("pes13hnolinq5_datrestr_1_def", 
                 "pes13hnolinq5_sexe_mod_datrestr_1.rda"),
     version = 2)
