...
for (i in pedepvars %>% 
  ...
  assign(paste("pes13nppsfe_pcs_reg_mod_", j, "_",
               i,
               "_tous_", k,
               sep = ""),
         feols(as.formula(paste(
           i, " ~ ",
           c("(pnp_score_norm : p) * pcs_reg_mod",
             "(pop_score_norm : p) * pcs_reg_mod",
             "score_norm",
             get(j)) %>% 
             paste(collapse = " + "),
...
save(list = ls()[str_detect(ls(), "pes13") & 
                  ! str_detect(ls(), "^pes13")],
     file = here("pes13nppsfe_def", 
                 "pes13nppsfe_pcs_reg_mod.rda"),
     version = 2)
