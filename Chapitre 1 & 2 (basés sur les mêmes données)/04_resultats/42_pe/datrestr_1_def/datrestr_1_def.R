...
ecoles_clniv <- pval_ecoles[pval_ecoles < .017] %>% names %>%
  substr(1, 8) %>% table %>% 
  (function (t) t[t >= 1]) %>% names
  # .017 dans BM18. On décide d'enlever une école si elle
  # fait les classes de niveau sur au moins une année. 
  # On enlève 1 école. "9741088P". 
  # C'est beaucoup par rapport à la première version.
  # Il faut savoir quel test faire. 

# datrestr_1_def ----------------------------------------------------------

datrestr_1 <- filter(dat, 
