...
for (i in pedepvars %>% 
  ...
  assign(paste("pes13nofe_pcs_reg_mod_", j, "_",
               i,
               "_tous_", k,
               sep = ""),
         lm(as.formula(paste(
           i, " ~ ",
           c("(p_score_norm : p) * pcs_reg_mod", "score_norm",
             get(j)) %>% 
             paste(collapse = " + ")
         )),
         data = get(k)))
...
for (i in ls()[str_detect(ls(), "^pes13nofe")]) {
...
save(list = ls()[str_detect(ls(), "pes13nofe") & 
                  ! str_detect(ls(), "^pes13nofe")],
     file = here("pes13nofe_def", "pes13nofe_pcs_reg_mod.rda"),
     version = 2)
